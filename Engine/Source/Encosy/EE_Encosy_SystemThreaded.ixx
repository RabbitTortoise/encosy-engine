...
export module EE_Encosy_SystemThreaded;

export import EE_Encosy_SystemBase;
...
public:
	...
	void SystemThreadedInit(ThreadedTaskRunner* threadRunner, ThreadedPerEntityTaskRunner* threadPerEntityRunner, int threadCount)
	{
		...
		ThreadPerEntityRunner = threadPerEntityRunner;
		ThreadCount = threadCount;
		RuntimeThreadInfo = std::vector<SystemThreadInfo>(ThreadCount, SystemThreadInfo());
...
private:
		...
		ThreadComponentStorage<ComponentType>* threadCopyStorage)
	{
		...
		auto saver = [=]() {
			auto* wrStorage = &writeReadStorage->Storage;
			for (size_t thread = 0; thread < ThreadCount; thread++)
			{
				auto* threadStorage = &threadCopyStorage->Storage[thread];
				ThreadDataSaver(thread, std::as_const(wrStorage), threadStorage);
			}
			};
...
protected:
	...
	// Variables:
public:
	bool GetAllowDestructiveEditsInThreads() const { return AllowDestructiveEditsInThreads; }
