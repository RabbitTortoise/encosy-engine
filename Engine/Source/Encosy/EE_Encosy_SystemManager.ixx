...
export module EE_Encosy_SystemManager;

import EE_Encosy_SharedBetweenManagers;
import EE_Encosy_SystemBase;
...
public:
	...
	void InitSystem(T* system, std::string systemName)
	{
		...
		system->SystemThreadedInit(&ThreadRunner, &ThreadPerEntityRunner, GetThreadCount());
		system->Init();
	}
	...
	std::vector<std::vector<SystemID>> ReorderSystems(SystemType systemType)
	{
		...
		for (int syncPoint = static_cast<int>(SystemSyncPoint::First); syncPoint < static_cast<int>(SystemSyncPoint::Last); syncPoint++)
		{
			...
			for (auto& system : Systems)
			{
				...
				if (static_cast<int>(systemPtr->GetRunSyncPoint()) != syncPoint) { continue; }
			...
			for (auto& systemDeps : dependencies)
			{
				...
				if (systemPtr->GetRunAfterSpecificSystem() != "")
				{
					auto afterDependency = SystemNames.find(systemPtr->GetRunAfterSpecificSystem());
					if (afterDependency == SystemNames.end())
					{
						fmt::println("ERROR: System {} has invalid RunAfterSpecificSystem: {}", GetSystemNameById(systemPtr->GetID()), systemPtr->GetRunAfterSpecificSystem());
					}
			...
			for (auto& systemDeps : dependencies)
			{
				...
				if (systemPtr->GetRunBeforeSpecificSystem() != "")
				{
					auto beforeDependency = SystemNames.find(systemPtr->GetRunBeforeSpecificSystem());
					if (beforeDependency == SystemNames.end())
					{
						fmt::println("ERROR: System {} has invalid RunBeforeSpecificSystem: {}", GetSystemNameById(systemPtr->GetID()), systemPtr->GetRunBeforeSpecificSystem());
					}
			...
			while (depsCorrected)
			{
				...
				for (auto& systemDeps : runOrder)
				{
					...
					if (systemPtr->GetRunWithSpecificSystem() != "")
					{
						auto withDependency = SystemNames.find(systemPtr->GetRunWithSpecificSystem());
						if (withDependency == SystemNames.end())
						{
							fmt::println("ERROR: System {} has invalid RunWithSpecificSystem: {}", GetSystemNameById(systemPtr->GetID()), systemPtr->GetRunWithSpecificSystem());
						}
